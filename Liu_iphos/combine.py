...
parts_dict = dict(zip(parts_df["parts"], parts_df["smiles"]))

# Function to process each lipid_name
def process_lipid(lipid_name):
    try:
        # Split into core and tail parts
        core_part, tail_part = lipid_name.split("-")
        
        # Look up SMILES for each part
        core_smiles = parts_dict.get(core_part, "")
        tail_smiles = parts_dict.get(tail_part, "")
